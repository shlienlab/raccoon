...
def _plotScore(scores, cparmrange, interface, name='./scores.png', path=""):
    ...
    Args:
        ...
        cparmrange (ndarray, list of floats): the clustering parameter exploration range.
    ...
    ax=plt.gca()
    maxs=[interface.num.amax(sc) for sc in scores]
    for i in range(len(scores)):
        if i==interface.num.argmax(maxs):
            ax.plot(cparmrange, scores[i], linewidth=1.5, color='#ff7149', zorder=101, label='best parm set')
            ax.scatter(cparmrange[interface.num.argmax(scores[i])], interface.num.amax(scores[i]), s=150, facecolor='none', color='#ff7149', zorder=101, lw=1.5)
        else:
            ax.plot(cparmrange, scores[i], linewidth=1, color='#AAAAAA', zorder=100)

    plt.tick_params(labelsize=15)
    plt.ylim([-1.1,1.1])
    plt.yticks(interface.num.arange(-1,1.1,.25),interface.num.arange(-1,1.1,.25))
...
def plotViolin(vals, interface, name='./rpdd.png', path=""):
    ...
    stdev=[interface.num.nanstd(v) for v in vals]

    customp=[sns.cubehelix_palette(121)[int(s*1.0/interface.num.nanmax(stdev)*100)] if not interface.num.isnan(s) else 0 for s in stdev]
    ...
    for i in range(len(vals)):
        ...
        ticks=['clu #'+str(x) for x in interface.num.arange(len(vals))]
...
def plotMap(df, labels, interface, name='./projection.png', path=""):
    ...
    colors = cmap(interface.getValue(interface.num.linspace(0, 1, len(set(labels.values)))))
